...
if [ -f backend/config.yml ]; then
    echo -e "  ${GREEN}✓${NC} backend/config.yml (生产环境配置)"
else
    echo -e "  ${YELLOW}⚠${NC} backend/config.yml 不存在"
fi

if [ -f backend/config.dev.yml ]; then
    echo -e "  ${GREEN}✓${NC} backend/config.dev.yml (开发环境配置)"
else
    echo -e "  ${RED}✗${NC} backend/config.dev.yml 不存在"
fi

if [ -f web/app/.env ]; then
    echo -e "  ${GREEN}✓${NC} web/app/.env (生产环境配置)"
else
    echo -e "  ${YELLOW}⚠${NC} web/app/.env 不存在"
fi

if [ -f web/app/.env.dev ]; then
    echo -e "  ${GREEN}✓${NC} web/app/.env.dev (开发环境配置)"
else
    echo -e "  ${RED}✗${NC} web/app/.env.dev 不存在"
fi

if [ -f web/app/.env.local ]; then
    echo -e "  ${GREEN}✓${NC} web/app/.env.local (当前使用的配置)"
else
    echo -e "  ${YELLOW}⚠${NC} web/app/.env.local 不存在"
fi
echo ""
